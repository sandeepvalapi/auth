package ex.ms.auth.foundation.api;

import ex.ms.auth.foundation.SetupParent;
...
import static ex.ms.auth.api.UserController.USERS_PATH;

public class UserControllerTest extends SetupParent {

    private static final UserTO DEFAULT_USER_TO_BE_SAVED = UserTO.builder()
            .clientId(ClientControllerTest.DEFAULT_CLIENT_TO_BE_SAVED.getClientId())
    ...
    @Before
    public void setUp() {
        clientService.save(ClientControllerTest.DEFAULT_CLIENT_TO_BE_SAVED);
    }

    @After
    public void tearDown() {
        if (clientRegistrationService.listClientDetails().size() > 0) {
            clientRegistrationService.removeClientDetails(ClientControllerTest.DEFAULT_CLIENT_TO_BE_SAVED.getClientId());
        }
    ...
    public void succeedOnSavingUser() throws Exception {
        ...
        assertThat(users.get(0).getClientId()).isEqualTo(ClientControllerTest.DEFAULT_CLIENT_TO_BE_SAVED.getClientId());
    ...
    public void succeedOnEncodingPassword() throws Exception {
        ...
        final List<UserTO> allByClientId = userService.getUsersForClient(
                ClientControllerTest.DEFAULT_CLIENT_TO_BE_SAVED.getClientId(),
                PageRequest.of(0, 10));
